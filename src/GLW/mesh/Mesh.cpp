...
//	}
	                ...
	                it != sregex_iterator(); ++it)
	{
		...
		for (size_t i = 0; i < 9; i += 3)
		{
			std::tuple<size_t, size_t, size_t> result =
			                std::make_tuple(
			                                (match[1 + i].length()) ?
			                                                std::stoi(
			                                                                match[1
			                                ...
			                                (match[3 + i].length()) ?
			                                                std::stoi(
			                                                                match[3
			                                                                                + i]) :
			                                                0);
			auto a = std::find(vertexlink.begin(), vertexlink.end(),
			                result);
			if (a == vertexlink.end())
			{
...
//	l.push<unsigned short>(1);
	...
	std::vector<tuple<tuple<float, float>, tuple<float, float, float>>> outvertecies(
	                vertexlink.size());
	std::transform(vertexlink.begin(), vertexlink.end(),
	                outvertecies.begin(),
	                [&verts, &texcoords](
	                                const std::tuple<size_t, size_t, size_t>& val)
	                                {
		                                return std::make_tuple(texcoords[std::get<0>(val)-1], verts[std::get<1>(val)-1] );
	                                });
